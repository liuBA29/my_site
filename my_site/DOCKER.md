...
**Обновление кода бота без остановки сайта:** после деплоя (git pull и т.п.) перезапусти только контейнер бота — сайт (web) продолжит работать:
```bash
docker compose restart bot
```
Бот на несколько секунд перезапустится и будет уже с новым кодом.
