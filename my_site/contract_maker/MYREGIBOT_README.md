...
- В `.env` уже есть **TELEGRAM_BOT_TOKEN** (тот же токен, что для уведомлений о регистрации). Дополнительные токены не нужны.
- Бот должен быть запущен **на том же компе**, где крутится проект (или на сервере с доступом к той же БД).
